...
fn thread1() {
    ...
    let pid = riot_rs::thread::current_pid().unwrap();
    assert_eq!(riot_rs::thread::get_priority(pid), Some(5));
    // Other thread prios didn't change.
    assert_eq!(riot_rs::thread::get_priority(ThreadId::new(0)), Some(2));

    // Reset priority.
    riot_rs::thread::set_priority(pid, 1);
