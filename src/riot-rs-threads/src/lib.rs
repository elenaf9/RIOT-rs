...
impl Threads {
    ...
    //     todo!();
    //     // self.current_pid()
    //     //     .map(|pid| self.threads.lock()[usize::from(pid)])
    // }
    ...
    fn set_current_pid(&self, pid: ThreadId) {
        #[cfg(feature = "multi-core")]
        {
            self.current_threads.lock()[usize::from(core_id())] = Some(pid)
        }
        #[cfg(not(feature = "multi-core"))]
        {
            *self.current_thread.lock() = Some(pid)
        }
    }
    ...
    fn create(
        ...
        let thread = &mut self.threads.lock()[usize::from(thread_id)];
    ...
    fn set_state(&self, pid: ThreadId, state: ThreadState) -> ThreadState {
        ...
        if state == ThreadState::Running {
            self.runqueue.lock().add(pid, prio);
            ...
            #[cfg(not(feature = "multi-core"))]
            self.runqueue.lock().pop_head(pid, prio);
    ...
    fn add_current_thread_to_rq(&self) {
        ...
        if state == ThreadState::Running {
            self.runqueue.lock().add(current_pid, prio);
        }
    }
...
pub fn yield_same() {
    THREADS.with(|threads| {
        ...
        #[cfg(not(feature = "multi-core"))]
        if threads.runqueue.lock().advance(prio) {
            schedule()
        }
